/**
 * ***************************************************************************** Copyright (c)
 * 2013-2015 Sierra Wireless and others.
 *
 ...
 *
 * <p>Contributors: Zebra Technologies - initial API and implementation Sierra Wireless, - initial
 * API and implementation Bosch Software Innovations GmbH, - initial API and implementation
 * *****************************************************************************
 */
...
public class LeshanClientDemo {
  ...
  private static final String[] modelPaths = new String[] {"3303.xml", "12131.xml"};

  private static final int OBJECT_ID_TEMPERATURE_SENSOR = 3303;
  private static final int OBJECT_ID_POLYSENSE_SENSOR = 12131;
  ...
  public static void main(final String[] args) throws InterruptedException {
    ...
    options.addOption(
        "lh", true, "Set the local CoAP address of the Client.\n  Default: any local address.");
    options.addOption(
        "lp",
        true,
    ...
    options.addOption(
        ...
        true,
        String.format(
            "Set the LWM2M or Bootstrap server URL.\nDefault: localhost:%d.",
    ...
    if ((cl.hasOption("i") && !cl.hasOption("p")) || !cl.hasOption("i") && cl.hasOption("p")) {
      System.err.println(
          "You should precise identity and Pre-Shared-Key if you want to connect in PSK");
      formatter.printHelp(USAGE, options);
      return;
    }
    ...
    String serverURI;
    if (cl.hasOption("u")) {
      if (cl.hasOption("i")) serverURI = "coaps://" + cl.getOptionValue("u");
      else serverURI = "coap://" + cl.getOptionValue("u");
    } else {
      if (cl.hasOption("i")) serverURI = "coaps://localhost:" + LwM2m.DEFAULT_COAP_SECURE_PORT;
      else serverURI = "coap://localhost:" + LwM2m.DEFAULT_COAP_PORT;
    }
  ...
  public static void createAndStartClient(
      String endpoint,
      String localAddress,
      int localPort,
    ...
    initializer.setInstancesForObject(OBJECT_ID_POLYSENSE_SENSOR, new PolySenseSensor());
    List<LwM2mObjectEnabler> enablers =
        initializer.create(SECURITY, SERVER, DEVICE, LOCATION, OBJECT_ID_TEMPERATURE_SENSOR,OBJECT_ID_POLYSENSE_SENSOR);
    ...
    if (!needBootstrap) {
      if (pskIdentity == null) builder.disableSecuredEndpoint();
      else builder.disableUnsecuredEndpoint();
    }
    final LeshanClient client = builder.build();

    LOG.info(
        "Press 'w','a','s','d' to change reported Location ({},{}).",
